...
You know the following important details:  The message plaintexts are in English (very important) - They were produced from the same key (even better!)
...
```
nikhilh@ubuntu:~/overthewire/krypton/level3$ ssh -p 2222 krypton3@krypton.labs.overthewire.org
 _                     _              
...
Executing frequency analysis (on `www.quipqiup.com`) on the text provided by `found1`, we get the approximate sequence of letters (in decreasing order of frequency:
`EIATSONRHCLDPMUYFWBGKXJVQZ`

Making appropriate substitutions on `krypton4` text, we get:
`welld oneth eleve lyour passw ordis brute`
